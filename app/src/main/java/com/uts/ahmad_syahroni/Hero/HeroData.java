...
//file HeroData sebagai penampung data yang akan digunakan nantinya, dimana pada file ini terdapat 3 Data yang masing2 di bungkus dalam array,
public class HeroData {
    ...
    //Array Detail Hero
    private static String[] detailhero = {
...
//    fungsi untuk mengambil semua data yang berada pada array
    public static ArrayList<HeroModel> getListData() {
        ArrayList<HeroModel> list = new ArrayList<>();
        for (int position = 0; position < namaHero.length; position++) {
            HeroModel hero = new HeroModel();
            hero.setNama(namaHero[position]);
            hero.setDetail(detailhero[position]);
            hero.setPhoto(imgHero[position]);
            list.add(hero);
        }
        return list;
    }
